...
class EC2APIPlugin(base.Scenario):
    ...
    def _get_client(self, is_nova):
        ...
        ec2_client = botocoreclient.APIClientEC2(
            url, args['region'], args['access'], args['secret'])
	vpc_client = botocoreclient.APIClientEC2(
            url, args['region'], args['access'], args['secret'])
        return (ec2_client, vpc_client)

    def _run_both(self, base_name, func):
        (ec2_client, vpc_client) = self._get_client(True)
        with AtomicActionWithoutFirst(self, base_name + '_nova'):
            func(self, ec2_client)
        (ec2_client, vpc_client) = self._get_client(False)
