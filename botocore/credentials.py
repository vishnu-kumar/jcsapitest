...
class EnvProvider(CredentialProvider):
    METHOD = 'env'
    ACCESS_KEY = 'JCS_ACCESS_KEY_ID'
    SECRET_KEY = 'JCS_SECRET_ACCESS_KEY'
    # The token can come from either of these env var.
    # JCS_SESSION_TOKEN is what other JCS SDKs have standardized on.
    TOKENS = ['JCS_SECURITY_TOKEN', 'JCS_SESSION_TOKEN']
    ...
    def __init__(self, environ=None, mapping=None):
        ...
        :param mapping: An optional mapping of variable names to
            ...
            change the mapping of access_key->JCS_ACCESS_KEY_ID, etc.
...
class OriginalEC2Provider(CredentialProvider):
    ...
    CRED_FILE_ENV = 'JCS_CREDENTIAL_FILE'
    ACCESS_KEY = 'JCSSAccessKeyId'
    SECRET_KEY = 'JCSSecretKey'
    ...
    def load(self):
        ...
        """
        if 'JCS_CREDENTIAL_FILE' in self._environ:
            full_path = os.path.expanduser(self._environ['JCS_CREDENTIAL_FILE'])
            creds = self._parser(full_path)
            if self.ACCESS_KEY in creds:
                logger.info('Found credentials in JCS_CREDENTIAL_FILE.')
...
class SharedCredentialProvider(CredentialProvider):
    ...
    # jcs_security_token, but the SDKs are standardizing on jcs_session_token
    # so we support both.
    TOKENS = ['jcs_security_token', 'jcs_session_token']
