...
class KeyPairTest(base.EC2TestCase):
    ...
    def test_create_delete_key_pair(self):
        keyName = 'Test key2'
        resp, data = self.ec2_client.CreateKeyPair(KeyName=keyName)
        self.assertEqual(200, resp.status_code, base.EC2ErrorConverter(data))
        res_clean = self.addResourceCleanUp(self.ec2_client.DeleteKeyPair,
                                            KeyName=keyName)
        ...
        resp, data = self.ec2_client.DeleteKeyPair(KeyName=keyName)
        self.assertEqual(200, resp.status_code, base.EC2ErrorConverter(data))
    ...
    def test_describe_key_pairs(self):
        ...
        resp, data = self.ec2_client.DescribeKeyPairs(KeyNames=[keyName])
        self.assertEqual(200, resp.status_code, base.EC2ErrorConverter(data))
        ...
        resp, data = self.ec2_client.DescribeKeyPairs(
        ...
        resp, data = self.ec2_client.DescribeKeyPairs(KeyNames=['fake key'])
        self.assertEqual(400, resp.status_code, base.EC2ErrorConverter(data))
    ...
    def test_import_empty_key_pair(self):
        keyName = 'Test key'
        publicKey = ''
        resp, data = self.ec2_client.ImportKeyPair(KeyName=keyName,
                                               PublicKeyMaterial=publicKey)
        if resp.status_code == 200:
            self.addResourceCleanUp(self.ec2_client.DeleteKeyPair, KeyName=keyName)
        self.assertEqual(400, resp.status_code, base.EC2ErrorConverter(data))
        self.assertEqual('MissingParameter', data['Error']['Code'])

    @testtools.skipUnless(CONF.aws.run_incompatible_tests,
                          "Different error code")
    def __test_import_invalid_key_pair(self):
        keyName = 'Test key'
